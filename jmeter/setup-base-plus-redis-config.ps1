# Setup for B + S (Base + Redis) Configuration - Redis Enabled, Kafka Disabled
Write-Host "Setting up B + S (Base + Redis) Configuration..." -ForegroundColor Cyan
...
Set-Location ..\jmeter
Write-Host "[OK] Kafka and Zookeeper stopped" -ForegroundColor Green
...
Write-Host ""
Write-Host "[OK] B + S (Base + Redis) Configuration Ready!" -ForegroundColor Green
Write-Host ""
...
Write-Host "   4. Save results as: results-base-plus-redis.csv" -ForegroundColor White
