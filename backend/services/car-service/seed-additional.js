...
async function seedAdditionalCars() {
  try {
    ...
    console.log(`📊 Current cars in database: ${currentCount}`);

    // Calculate how many more we need
    // Target: 4,000 cars total (to reach 10,000+ total with flights and hotels)
    const targetCount = 4000;
    const additionalNeeded = Math.max(0, targetCount - currentCount);

    if (additionalNeeded === 0) {
      console.log('✅ Already have enough cars!');
      await mongoose.connection.close();
      return;
    }

    console.log(`🔄 Generating ${additionalNeeded} additional cars...`);
    ...
    for (let i = 0; i < additionalNeeded; i++) {
      const carId = `CAR${String(startIndex + i).padStart(4, '0')}`;
      ...
      if ((i + 1) % 100 === 0) {
        console.log(`   Generated ${i + 1}/${additionalNeeded} cars...`);
      }
    }
    ...
    console.log(`✅ Successfully seeded ${additionalNeeded} additional cars!`);
    console.log(`📊 Total cars now: ${finalCount}`);
